...
class tracking_apriltag(object):
	def __init__(self):
		...
		self.yaw = GPIO.PWM(yaw_pin, 90)


		# pwm_input_value, [0]=t, [1]=t-1, center_value=7.422
		self.pitch_input_pwm = 7.422
		self.yaw_input_pwm = 13.6#7.422#14.844#30.079#60.156
		self.stop_pwm = 7.422
	...
	def tag_image_callback(self, data_image):
		if len(data_image.detect_positions) >= 1:
			...
			self.yaw_input_pwm = 13.6#7.422#60.156
	...
	def yaw_pid_controller(self,event=None):
		...
		"""
		if self.yaw_input_pwm >= 10.742:#10.742#21.289#43.164#86.523
...
if __name__ == "__main__":
	...
	rospy.Timer(rospy.Duration(1.0/90), ts.yaw_pid_controller)
	#rospy.Timer(rospy.Duration(1.0/20), ts.timer)
